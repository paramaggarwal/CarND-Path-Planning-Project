...
// Get the closest map point given a point
...
// Closest map point from a point facing in a particular direction
int NextWaypoint(double x, double y, double theta, vector<double> maps_x, vector<double> maps_y, vector<double> maps_dx, vector<double> maps_dy)
{
    ...
    // vector into the direction of the road (prependicular to the normal vector)
    double vx = -ny;
    double vy = nx;
    
    // if the inner product of v and h is positive then we are behind the waypoint so we do not need to
    // increment closesWaypoint, otherwise we are beyond the waypoint and we need to increment closesWaypoint.
...
// Convert to map lane/distance from x/y/angle points
...
/* Given an x/y/angle point, convert it to map's lane/distance format */
